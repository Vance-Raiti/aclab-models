...
class Model(torch.nn.Module):
    def __init__(self,n_blocks,n_steps,in_channels):
        ...
        self.loss_fn = torch.nn.PoissonNLLLoss(log_input=False,full=True,reduction='mean')
    ...
    def forward(self,seq, targets=None):
        ...
        '''
        x = self.blocks(seq)
        out = self.linear(torch.transpose(x,0,1))
        y_hat = self.softplus(torch.squeeze(torch.transpose(out,0,1),0))
...
def encode_sequence(seq):
    ...
    out = torch.transpose(torch.nn.functional.one_hot(seq,4),0,1)
